...
@if( isset($_GET['workday']) )
    <style type="text/css">
        #projectDetailsContainer-table th:first-child,#projectDetailsContainer-table td:first-child{
                    ...
                    <form action="{{route('dayWorkHour.store')}}" method="POST" class="form-horizontal" id="dayWorkHour">
                        ...
                        <div id="project-dropdown-container">
                            <div class="form-group{{ $errors->has('workDayDate') ? ' has-error' : '' }} ">
                                ...
                                <div class="col-md-8">
                                    <input type="text" id="workDayDate" name="workDayDate" value="{{ (isset($_GET['workday']))?$_GET['workday']:'' }}" class="form-control" getWorkEntriesByDate-route="{{ route('getWorkEntriesByDate') }}">
...
(function(){
    ...
    function returnData(uId=0) {
        return new Promise(resolve => {
            $.ajax({
                url: 'ajaxAssignedUserProject',
                data: {uId: uId},
                method: 'get',
                dataType: 'json'
